...
### step 0: get config path
config location 
- by default config is in `~/.config/opencode/`
- but if `$OPENCODE_CONFIG_DIR` is set use that instead

```bash
OC_PATH=$(echo $OPENCODE_CONFIG_DIR)

if [ -z "$OC_PATH" ]; then
  OC_PATH="~/.config/opencode"
fi
...
2. **Infer patterns** for **core code/config files ONLY** to include:
   ...
   - **Exclude (MANDATORY)**: Do NOT include tests, documentation, or translations.
     ...
     - graphfify: `graphify-out/`
   - Respect `.gitignore` automatically
3. **Run codemap.mjs init**:

```bash
bun run $OC_PATH/skills/codemap/scripts/codemap.mjs init \
  --root ./ \
  --include "src/**/*.ts" \
...
```bash
bun run $OC_PATH/skills/codemap/scripts/codemap.mjs changes \
  --root ./
```
...
```bash
bun run $OC_PATH/skills/codemap/scripts/codemap.mjs update \
  --root ./
```
